...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input", default="input", metavar="DIR", help="Input folder (default: input/)"
    )
    parser.add_argument(
        "--output", default="output", metavar="DIR", help="Output folder (default: output/)"
    )
    parser.add_argument(
        "--excel", default="passwords.xlsx", metavar="FILE", help="Excel/CSV mapping file"
    )
    parser.add_argument(
        "--log-dir", default="logs", metavar="DIR", help="Log directory (default: logs/)"
    )
    parser.add_argument(
        "--workers", type=int, default=4, metavar="N", help="Parallel worker threads (default: 4)"
    )
    parser.add_argument(
        "--skip-encrypted", action="store_true", help="Skip PDFs that are already encrypted"
    )
    parser.add_argument(
        "--no-overwrite", action="store_true", help="Do not overwrite existing output files"
    )
