...
class ExcelReader:
    ...
    def read(self) -> ExcelReadResult:
        ...
        logger.info("Using columns -> file: '%s', password: '%s'", file_col, pass_col)
    ...
    def _validate_file_exists(self) -> None:
        if not self._filepath.exists():
            raise FileNotFoundError(f"Excel/CSV mapping file not found: {self._filepath}")
    ...
    def _load_dataframe(self) -> pd.DataFrame:
        ...
        try:
            if suffix == ".csv":
                ...
                df = pd.read_excel(self._filepath, dtype=str, keep_default_na=False)
        except Exception as exc:
            raise ValueError(f"Failed to read mapping file '{self._filepath}': {exc}") from exc
    ...
    def _parse_rows(self, df: pd.DataFrame, file_col: str, pass_col: str) -> ExcelReadResult:
        ...
        for idx, row_data in enumerate(df.itertuples(index=False), start=1):
            ...
            normalised = (
                raw_filename if raw_filename.lower().endswith(".pdf") else raw_filename + ".pdf"
            )
            ...
            if not raw_password or raw_password.lower() == "nan":
                ...
                logger.warning("Row %d skipped: empty password for '%s'", idx, normalised)
                continue
