    ...
    def __init__(
        ...
        **kwargs,
        ...
        tk.Label(
            ...
            bg=COLORS["bg_card"], fg=COLORS["text_muted"], width=16, anchor="w"
        ).pack(side="left", padx=(0, 8))

        self._var = tk.StringVar()
        entry = tk.Entry(
            self, textvariable=self._var, font=FONTS["input"],
        ...
        btn = tk.Button(
            self, text="Browse…", font=FONTS["btn_sm"],
            bg=COLORS["accent_dark"], fg=COLORS["text"],
...
class _StatCard(tk.Frame):
    ...
    def __init__(self, parent: tk.Widget, label: str, color: str, **kwargs) -> None:
        ...
        tk.Label(
            ...
            font=FONTS["stat"], bg=COLORS["bg_card"], fg=color,
        ).pack()
        tk.Label(
            self, text=label,
            font=FONTS["stat_lbl"], bg=COLORS["bg_card"], fg=COLORS["text_muted"],
        ).pack()
...
class PDFProtectorApp:
    ...
    def _build_paths_card(self, parent: tk.Widget) -> None:
        ...
        for row in (self._input_row, self._output_row,
                    self._excel_row, self._log_dir_row):
            row.pack(**pad)
    ...
    def _build_settings_card(self, parent: tk.Widget) -> None:
        ...
        # Workers
        tk.Label(row, text="Parallel Workers:", font=FONTS["label"],
                 bg=COLORS["bg_card"], fg=COLORS["text_muted"]).pack(side="left")
        self._workers_var = tk.IntVar(value=4)
        workers_spin = tk.Spinbox(
            row, from_=1, to=32, textvariable=self._workers_var,
            width=4, font=FONTS["input"],
            bg=COLORS["bg_input"], fg=COLORS["text"],
    ...
    def _build_stats_row(self, parent: tk.Widget) -> None:
        ...
        (self._stat_total, self._stat_encrypted,
         self._stat_skipped, self._stat_missing,
         self._stat_failed) = cards
    ...
    def _build_progress_section(self, parent: tk.Widget) -> None:
        ...
        tk.Label(
            top, text="Progress", font=FONTS["label"],
            bg=COLORS["bg_card"], fg=COLORS["text_muted"],
        ).pack(side="left")

        self._progress_label = tk.Label(
            top, text="Ready", font=FONTS["label"],
            bg=COLORS["bg_card"], fg=COLORS["text_muted"],
        )
        ...
        tk.Label(
            ...
            font=FONTS["mono"], bg=COLORS["bg_card"], fg=COLORS["text_muted"],
    ...
    def _build_buttons(self, parent: tk.Widget) -> None:
        ...
        self._stop_btn = tk.Button(
            ...
            activebackground=COLORS["error"], activeforeground=COLORS["text"],
            relief="flat", bd=0, cursor="hand2",
            padx=20, pady=10,
            state="disabled",
    ...
    def _poll_queues(self) -> None:
        ...
        encrypted = skipped = missing = failed = 0
        while not self._result_queue.empty():
            ...
            self._append_log_direct(
                f"  {r.status.upper():<8}  {r.filename}  — {r.message}", tag
            )
    ...
    def _reset_stats(self) -> None:
        for card in (self._stat_total, self._stat_encrypted,
                     self._stat_skipped, self._stat_missing,
                     self._stat_failed):
            card.set(0)
    ...
    def _show_summary_dialog(self, summary: RunSummary) -> None:
        ...
        icon_lbl = tk.Label(
            dlg, text="✓" if summary.failed == 0 else "⚠",
        ...
        for i, (lbl, val, clr) in enumerate(rows_data):
            tk.Label(
                grid, text=lbl, font=FONTS["label"],
                bg=COLORS["bg_card"], fg=COLORS["text_muted"],
                anchor="w", padx=16, pady=6,
            ).grid(row=i, column=0, sticky="w")
            tk.Label(
                grid, text=str(val), font=("Segoe UI", 10, "bold"),
                bg=COLORS["bg_card"], fg=clr, padx=16,
